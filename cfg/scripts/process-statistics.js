...
var asctime = function (duration) {
    var milliseconds = Math.floor(duration % 1000), seconds = Math.floor((duration / 1000) % 60), minutes = Math.floor((duration / (1000 * 60)) % 60), hours = Math.floor((duration / (1000 * 60 * 60)) % 24), days = Math.floor(duration / (1000 * 60 * 60 * 24));
...
var TableForm = function (_a) {
    var api = _a.api;
    var _b = reactExports.useState(0), active = _b[0], setActive = _b[1];
    var _c = reactExports.useState([]), instances = _c[0], setInstances = _c[1];
    var _d = reactExports.useState(items[0].defaultMaxResults), maxResults = _d[0], setMaxResults = _d[1];
    var _e = reactExports.useState(items[0].selectMessage), selectMessage = _e[0], setSelectMessage = _e[1];
    var _f = reactExports.useState(items[0].selectMaxResult), selectMaxResult = _f[0], setSelectMaxResults = _f[1];
    var _g = reactExports.useState(''), filterVar = _g[0], setFilterVar = _g[1];
    var _h = reactExports.useState(''), filterValue = _h[0], setFilterValue = _h[1];
    // FETCH
